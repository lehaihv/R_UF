...
set.seed(1234)
##################################################
# covid_1 <- as.data.table(read_excel("~/Documents/GitHub/R_UF/ML/Key_WW/covid_cdc_cases_03202024_100k_key_ww.xlsx", sheet = 2))
# covid_2 <- as.data.table(read_excel("~/Documents/GitHub/R_UF/ML/Key_WW/covid_cdc_cases_03202024_100k_key_ww.xlsx", sheet = 3))
# covid_3 <- merge(covid_1, covid_2, by = "key_sewershed")
# write_xlsx(covid_3, "~/Documents/GitHub/R_UF/ML/Key_WW/covid_cdc_cases_03202024_100k_key_ww_full.xlsx")
...
covid_cases_cdc <- covid_cases_cdc[, -c("absolute_values", "population_served")]
...
for (z in 301:373) { #length(same_county$county_names)
  ...
  if (length(full_cdc_cases$sample_collect_date)>50) {
    ################################################################
    # Create example data
    ...
    # filename=paste("~/Documents/GitHub/R_UF/ML/Key_WW/CC_data_update/", same_county$county_names[z],"_CC_data.xlsx")
    # write_xlsx(full_cdc_cases, filename) 
    ###############################################
    # viral concentration
    ...
    if (length(full_ww_virus$sample_collect_date)>50) {
      ################################################################
      # Create example data
      ...
      # filename=paste("~/Documents/GitHub/R_UF/ML/Key_WW/WW_data_update/", same_county$county_names[z],"_WW_data.xlsx")
      # write_xlsx(full_ww_virus, filename)
      ...
      if (length(join_data$sample_collect_date) >50){
        ...
        # filename=paste("~/Documents/GitHub/R_UF/ML/Key_WW/Join_data_update/", same_county$county_names[z],"_Join_data.xlsx")
        # write_xlsx(join_data, filename)
        # ################################################################
        # # save graph of each sewersheds to file
        # png(filename=paste("~/Documents/GitHub/R_UF/ML/Key_WW/Graphs_update/", same_county$county_names[z],".png"),
        #     width	= 2050,
        #     height = 1020,
        #     res = 180)
...
# write_xlsx(df_mul, "~/Documents/GitHub/R_UF/ML/Key_WW/Join_data_update/Viral_activity_overlap_time_373_sewersheds.xlsx")
