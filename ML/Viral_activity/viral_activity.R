...
for (z in 1:length(same_county$county_names)) { # 226length(same_county$county_names)
  ...
  # full_cdc_cases$full_cdc_cases_Index <- as.numeric(row.names(full_cdc_cases))
  # temp <- full_cdc_cases$full_cdc_cases_Index[]
  temp <- which(grepl(last_day, full_cdc_cases$sample_collect_date))
  #temp[1]
  quantile_cdc_cases_10 = quantile(full_cdc_cases$lowess_data[1:temp[1]], probs = c(0.1))
  ################################################################
  ...
  if (full_range > (window_size + 1)) {
    ...
    # Calculate trends
    for (x in (window_size + 1):full_range) {
      if(full_cdc_cases$lowess_data[x] > full_cdc_cases$lowess_data[x - window_size] * slope_high) { 
        full_cdc_cases$trends[x] = 1
      } else if(full_cdc_cases$lowess_data[x] < full_cdc_cases$lowess_data[x - window_size] * slope_low){ 
        full_cdc_cases$trends[x] = -1
      }
  ...
  if (length(virus_county$sample_collect_date) > (window_size + 1)) {
    ...
    if (full_range > (window_size + 1)) {
      ...
      # Calculate trends
      for (x in (window_size + 1):full_range) {
        if(full_ww_virus$lowess_data_virus[x] > full_ww_virus$lowess_data_virus[x - window_size] * slope_high) { 
          full_ww_virus$trends_virus[x] = 1
        } else if(full_ww_virus$lowess_data_virus[x] < full_ww_virus$lowess_data_virus[x - window_size] * slope_low){ 
          full_ww_virus$trends_virus[x] = -1
        }
